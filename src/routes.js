...
router.get("/bookings/", async (req, res, next) => {
  ...
  if (isNaN(Date.parse(date)))
    return res.status(422).send({ errorCode: "DATE_BAD_FORMAT" });
...
router.put("/bookings/:id", async (req, res, next) => {
  ...
  if (date && isNaN(Date.parse(date))) errors.push({ date: "BAD_FORMAT" });
  ...
  if (errors.length > 0)
    return res.status(422).send({ errorCode: "VALIDATION_ERROR", errors });
  ...
  if (!updatedBooking)
    return res.status(404).send({ errorCode: "BOOKING_NOT_FOUND" });
